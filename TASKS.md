...
（在看到这个文档之后，**立刻用 TodoWrite 记录下面八项，在记录之前不准开始任何行动！如果没有 TodoWrite 工具，你应该直接向用户提问，而不是进行任何行动！**）
...
已完成：迁移到 uv 新结构（src/hdr/, tests/, examples/），配置 pyproject.toml + hatchling，uv run pyright 和 pytest 均通过。
